...
void introduce(struct student s) {
	printf("안녕하세요 저는 %d번 %d살 %s이고요, 저의 짝궁은 %d번 %d살 %s입니다.", s.num, s.age, s.name, s.friend->num, s.friend->age, s.friend->name);
}

int main(void) {
	struct student s2 = { 3,22,"황진이" };
	struct student s3 = { 2,21,"홍길순" };
	struct student s1 = { 1,20,"홍길동",&s2 };
